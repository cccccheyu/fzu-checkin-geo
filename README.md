...
> 🖱️ **不想手填配置？用[网页版配置生成器](https://27fd7ecd5c584e868e19e4f8d714f702.app.workbuddy.link)**：填完账号 / 位置 / 推送三步后，按「有没有 GitHub 账号」二选一 —— **有账号就贴个 Token 一键部署**（自动替你建私有仓库、写入代码和你的配置），没账号跟着分步引导走。另有**补假期工具**（只选起止日期即可，还能一键写回你的仓库）和**本机记忆**（填过的内容下次打开自动带出）。纯前端页面，不收集任何数据，也不记密码 / token。也可以把仓库里的 `web/configurator.html` 下载后双击离线使用。下面的手动流程照旧可用。
>
> 📣 **想拉同学一起用？发[宣传卡](https://d1d0d89e2c9940728b8454871388ef26.app.workbuddy.link)**：一页讲清痛点 + 卖点 + 二维码，扫码直达配置页，也可以直接转发链接。仓库里的 `web/promo.html` 就是同一张卡，可离线打开。
